...
webserver.get('/api/[a-z]+', function (req, res, next) {
	...
	if (req.query.path) {
		...
		try {
			...
			// check permissions
			if (!perms.test(res.locals.userPerms, queryPath)) {
				throw 'User do not have permissions to path"' + queryPath + '"'; // user dont have permission to this path
			}
			let fullPath = path.join(c.path, queryPath).replaceAll('\\', '/');
...
webserver.get('/api/search', function (req, res) {
	...
	readdirp(res.locals.fullPathFolder, {type: 'files_directories', depth: 10, alwaysStat: false}).on('data', function (entry) {
		try {
			if (!entry.dirent.isDirectory() && !entry.basename.match(c.extRegex)) {
				return; // not directory or not match allowed extension
			}
			let entryPath = entry.fullPath.replace(/\\/g, '/').replace(c.path, '/'); // all folder separators has to be /
...
webserver.get('/api/structure', function (req, res) {
	...
	const loadFilesPromise = new Promise(function (resolve) {
		...
		function getCoordsFromExifFromFile(fullPath) {
			try {
				if (fullPath.match(c.extExifRegex) === false)  {
					return {};
				}
		...
		globby(globSearch[1], {nodir: true}).then(function (rawPathsFiles) {
			rawPathsFiles.forEach(function (fullPath) {
				try {
					...
					const dynamicPath = fullPath.replace(c.path, '/');
					if (perms.test(res.locals.userPerms, dynamicPath)) {
						if (pathStats.isFile() && dynamicPath.match(c.extRegex)) {
							let pathData = {
								path: dynamicPath,
								size: pathStats.size,
